	...
	if (!file_exists('iPmart.install')) {
		if (file_exists('../config.php') and file_exists('../index.php') and file_exists('../texts.json') and file_exists('../sql/sql.php')) {
			...
			if ($getTokenStatus == true) {
				...
				file_put_contents('iPmart.install', json_encode(['development' => '@iPmartPanel', 'install_location' => $_POST['install_location'], 'main_domin' => $domain, 'token' => $_POST['token'], 'dev' => $_POST['admin-id'], 'db_name' => $_POST['db-name'], 'db_username' => $_POST['db-user'], 'db_password' => $_POST['db-pass']], 448));
